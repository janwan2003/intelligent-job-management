...
profile sweep and~the~deadline-driven URGENT migration sequence on~an
under-subscribed cluster.  Scenario~2 mixes two job types and~fills
every slot, pitting URGENT against priority-5 pins on~A40 and~slack
lstm-small patients on~P600.  Together they separate two behaviours of
the~upstream GPUspb proxy: Scenario~1 exposes its \emph{horizon-myopia}
(an~urgent job parked on~slow but~free hardware), while Scenario~2 shows
that on~a~full cluster the~right outcome still emerges from~its~stateless
cost minimisation: the~cheapest slack work is~omitted from~the~plan
(realised as~a~preemption) and, once an~equal-priority pin finishes, the
urgent job migrates onto the~freed fast~A40 and~meets its deadline.
