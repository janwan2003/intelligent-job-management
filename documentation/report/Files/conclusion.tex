...
Three follow-ups have concrete entry points in~the~codebase.  First,
swap the~Docker executor for~SLURM: the~\texttt{Executor} abstraction
in~\texttt{backend/src/job\_runner.py} already isolates the~Docker
calls, and~the~worker's HTTP surface is~small enough to~back with~a
SLURM batch-submit shim.  Second, re-run Scenarios~1 and~2 with
...
into it~over two patient-completion events.  Third, extend the
