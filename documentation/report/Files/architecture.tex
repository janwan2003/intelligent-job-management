...
\begin{enumerate}[leftmargin=*,itemsep=2pt,topsep=4pt]
  ...
  \item \textbf{Profile placement.}
    ...
    \texttt{ProfilingScheduler} atomically claims one unmeasured
    (node, GPU-count) cell using a~best-fit-on-remaining-capacity rule.
    This is~the~mechanism behind the~\emph{profile-always} policy: an
    instance must complete \texttt{PROFILING\_CONFIGS\_PER\_JOB}
    profile runs (default~2) before it~becomes a~candidate for~the
    optimiser-driven standard placement above.
...
released when the~worker emits a~\texttt{NOTIFY ijm\_slot\_freed}
payload of~the~form
\texttt{<node\_id>:<n\_gpus>:<reason>}.  The~listener always releases
the~permit; whether it~also wakes the~optimiser is~gated by~the
reason.  \texttt{terminal} (job ended naturally), \texttt{user\_stop}
(UI-initiated), and~\texttt{orphan\_drain} (zombie cleanup) are
external events and~trigger an~optimiser pass.  \texttt{auto\_preempt}
is~the~API's own \texttt{/stop} issued as~part of~an~in-flight plan;
the~plan's own dispatch step will pick up~the~just-released slot, so
re-running the~optimiser there would thrash the~plan it~is~already
executing.  The~reason is~set at~the~worker's emit site
(\texttt{SlotFreedReason} in~\texttt{shared/constants.py}).
