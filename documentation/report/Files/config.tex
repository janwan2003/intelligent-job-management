...
The fields above describe what the~API \emph{stores}; they say nothing about
what the~container must \emph{do}.  Any image is~schedulable, but it~becomes
\emph{stoppable, resumable, and~migratable} only if~its entrypoint honours
a~small contract.  Every training image in~this project satisfies it by
subclassing \texttt{BaseTrainer} (\texttt{runtime/base.py}) and~implementing
just three hooks~--- \texttt{\_create\_model}, \texttt{\_load\_datasets},
and~\texttt{\_preprocess\_batch}; the~base class supplies everything in
Table~\ref{tab:job-contract}, whose mechanism is~detailed
