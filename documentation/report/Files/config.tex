...
non-standard workloads.  The~request mixes naming conventions~---
\texttt{job\_id} (snake\_case) alongside camelCase \texttt{dockerImage},
\texttt{scriptPath}, \texttt{batchSize}, and~a~capitalised \texttt{Priority}~---
a~carry-over from~ANDREAS-compatible field names; the~API accepts them through
Pydantic aliases and~normalises to~snake\_case internally, which is~why the
stored object and~the~201 response below are~uniformly snake\_case.
