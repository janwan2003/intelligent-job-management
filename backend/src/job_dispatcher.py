...
class JobDispatcher:
    ...
    async def stop(self, job_id: str) -> None:
        ...
        worker_url = self.get_worker_url(node_id)
        if worker_url:
            asyncio.create_task(self._remote_request(worker_url, job_id, "stop"), name=f"remote-stop-{job_id[:8]}")
        else:
            await self._local.stop(job_id)

    async def _remote_request(self, worker_url: str, job_id: str, action: str) -> None:
        try:
            async with httpx.AsyncClient(timeout=_HTTP_TIMEOUT) as client:
                resp = await client.post(f"{worker_url}/jobs/{job_id}/{action}")
                resp.raise_for_status()
            logger.info("Remote %s for job %s on %s", action, job_id[:8], worker_url)
        except Exception as exc:
            logger.warning("Remote %s failed for job %s on %s: %s", action, job_id[:8], worker_url, exc)
