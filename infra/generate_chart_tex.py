...
def _parse_submit_times(api_log: Path, anchor_day: datetime) -> dict[str, datetime]:
    ...
    Applies the same TZ offset ``parse_log`` infers (UTC-in-container
    vs.~CEST-on-host), so the returned timestamps live in the same
    coordinate system as the segment timestamps; without this the
    deadline overrides resolve to negative minutes.
    """
    ...
    for raw in raw_lines:
        ...
        if jid in out:
            continue
        out[jid] = parse_time(ts_s, anchor_day) + api_offset
    return out
